...
def TwoFieldEmbed(Title, FieldOne, FieldTwo, EmbedColor=Default_Color, FooterText=Name, FooterIconURL=BotImage, i1=True, i2=True):
    ...
    embedVar = discord.Embed(title=str(Title), color=EmbedColor)
    embedVar.add_field(name=str(FieldOne[0]), value=str(FieldOne[1]), inline=i1)
    embedVar.add_field(name=str(FieldTwo[0]), value=str(FieldTwo[1]), inline=i2)
    embedVar.set_footer(text=FooterText, icon_url=FooterIconURL)
    return embedVar

def ThreeFieldEmbed(Title, FieldOne, FieldTwo, FieldThree, EmbedColor=Default_Color, FooterText=Name, FooterIconURL=BotImage, i1=True, i2=True, i3=True):
    """ A basic embed that has a name, title, and three fields for text 
        Takes Field values in a double, 0 being the name and 1 being the value. """
    embedVar = discord.Embed(title=str(Title), color=EmbedColor)
    embedVar.add_field(name=str(FieldOne[0]), value=str(FieldOne[1]), inline=i1)
    embedVar.add_field(name=str(FieldTwo[0]), value=str(FieldTwo[1]), inline=i2)
    embedVar.add_field(name=str(FieldThree[0]), value=str(FieldThree[1]), inline=i3)
    embedVar.set_footer(text=FooterText, icon_url=FooterIconURL)
    return embedVar